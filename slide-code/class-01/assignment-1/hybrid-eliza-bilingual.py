...
## Dataset link (EN): https://www.kaggle.com/datasets/bhavikjikadara/emotions-dataset
...
try:
    # oppaWord-inspired tokenizer (see mmdt-tokenizer)
...
class HybridEliza:
    def __init__(self, lang="en", model_path=None):
        ...
        self.model = None
        self.mya_tokenizer = MyanmarTokenizer() if (self.lang == "mya" and MyanmarTokenizer) else None

    def _tokenize(self, text):
        if self.lang == "mya":
            if self.mya_tokenizer:
                tokens = self.mya_tokenizer.word_tokenize(text)
                ...
                for t in tokens:
                    if isinstance(t, list):
                        ...
                        flat.append(str(t))
                return flat
            # Simple fallback: if tokenized with spaces, use them; otherwise fall back to character tokens.
            cleaned = re.sub(r"[^\w\s\u1000-\u109F]", " ", text)
...
def main():
    ...
    parser.add_argument("--lang", default="en", choices=["en", "mya"])
